...
public class FragmentData
{
    ...
    public FragmentData(Mesh mesh)
    {
        ...
        {
            // UV koordinatlarý varsa, kullanýn; yoksa varsayýlan deðer olarak (0, 0) kullanýn
            Vector2 uvValue = uv != null && uv.Length > i ? uv[i] : new Vector2(0, 0);
            this.Vertices.Add(new MeshVertex(positions[i], normals[i], uvValue));

            //this.Vertices.Add(new MeshVertex(positions[i], normals[i], uv[i])); original kod
        }
