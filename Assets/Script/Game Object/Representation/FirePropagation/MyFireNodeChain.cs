...
public class MyFireNodeChain : MonoBehaviour {
    ...
    public MyFireNode[] m_fireNodes = null;
    ...
    private void OnCollisionEnter(Collision collision)
    {
        ...
        if(collision.transform.CompareTag("Ammo") && collision.gameObject.GetComponent<Ammo>()?.matter==AmmoMatter.Fire)
        {
            ...
            m_fireNodes[index].m_fireJustStarted=true;
        }
    }
    ...
    void PropagateFire()
    {
        ...
        for (int i = 0; i < m_fireNodes.Length; i++)
        {
            if (m_fireNodes[i].isAlight)
            {
                ...
                {
                    if (m_fireNodes[i].m_links[child] != null && m_fireNodes[i].m_links[child].GetComponent<MyFireNode>().HP > 0.0f)
                    {
                        m_fireNodes[i].m_links[child].GetComponent<MyFireNode>().HP -= m_firePropagationSpeed * Time.deltaTime;
                    }
                }
            }
